...
// 1. Define Interface to match BotBuilder requirements
interface BacktestPanelProps {
  code?: string; // The Python code generated by AI
  parameters?: any; // The params tuned in Step 2
  onResults?: (results: any) => void; // Callback to unlock Step 4
  onExport?: () => void; // Callback to move to Step 4
}
...
export default function BacktestPanel({ 
  ...
  // Auto-run when component mounts if we have parameters
  // Optional: You can remove this useEffect if you prefer manual start
  useEffect(() => {
    if (Object.keys(parameters).length > 0 && !results) {
      // handleRunBacktest(); // Uncomment to auto-run
    }
  }, [parameters]);
  ...
  const handleRunBacktest = useCallback(async () => {
    ...
    try {
      // 1. Fetch Real Data
      // Note: You might want to make the symbol dynamic based on parameters too
      const symbol = parameters.symbol || 'ES.c.0';
      const response = await fetch(`/api/market-data?symbol=${symbol}`);
      
      if (!response.ok) {
        // Fallback for demo if API isn't built yet
        console.warn("API failed, using mock data for demo purposes");
        // throw new Error('Failed to fetch market data'); 
      }

      // Mock data generation if API fails (Prevent "Stop" while building)
      let candles: Candle[] = [];
      if (response.ok) {
        const data = await response.json();
        candles = data.candles;
      } else {
        // Generate synthetic data so the UI works
        const now = new Date();
        let price = 5000;
        for (let i = 0; i < 365; i++) {
            price = price * (1 + (Math.random() - 0.45) * 0.02);
            candles.push({
                time: new Date(now.getTime() - (365 - i) * 86400000).toISOString(),
                open: price,
                high: price * 1.01,
                low: price * 0.99,
      ...
      // 2. Define Strategy (Dynamic based on Props!)
      const strategy: StrategyFunction = (candle, index, allCandles) => {
        // Use parameters from props, default to 9/21 if missing
